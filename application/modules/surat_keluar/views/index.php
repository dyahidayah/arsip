    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <table id="table_target" class="table table-bordered bg-white">
                ...
                <?php 
                if($data_suratkeluar):
                foreach($data_suratkeluar as $data) :?>
                        <tr>
                            <td class="no_kategori text-center">
                                <a class="view_detail text-dark font-weight-bold" href="<?= site_url('surat_keluar/detail_surat/').$data->no_agenda ?>" alt="Detail Surat">
                                    <?= $data->no_surat?>
                                </a>
                            </td>
    ...
    function delete_surat(no_agenda)
    {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: base+'surat_keluar/delete_surat',
                    type: 'post',
                    dataType: 'json',
                    data: {no_agenda: no_agenda},
                })
                .done(function(data) {
                    location.href = base+'surat_keluar/';
                })
                .fail(function() {
                    Swal.fire(
                        'OPSS',
    ...
    function recovery(no_agenda)
    {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var active = $('#active').val()
                $.ajax({
                    url: base+'surat_keluar/recovery',
                    type: 'post',
                    dataType: 'json',
                    data: {no_agenda: no_agenda},
                })
