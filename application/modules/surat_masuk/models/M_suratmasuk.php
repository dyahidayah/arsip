...
class M_suratmasuk extends CI_Model
{
    ...
    function delete_suratmasuk() {
        ...
        $cek = $this->db->update('surat_masuk', $data);
    ...
    function generate_number_agenda()
    {
        ...
        $res1 = $this->db->get('surat_masuk')->row();
    ...
    public function simpan_suratmasuk()
    {
        ...
        $cek_exist = $this->db->get_where('surat_masuk', $where)->row();

        if (!$cek_exist) {
            //upload PDF
            $data['file'] = $this->upload();
			$cek = $this->db->insert('surat_masuk', $data);
           
			if ($cek) {
				$res = [
					'kode'		=> 'success',
					'msg'		=> 'Surat Masuk Berhasil Disimpan',
				];
			} else {
				$res = [
					'kode'		=> 'error',
    ...
    public function simpanedit_suratmasuk($no_agenda)
    {
        ...
        if ($cek) {
			...
			if ($cek) {
				$res = [
					'kode'		=> 'success',
					'msg'		=> 'Surat Masuk Berhasil Diperbaiki',
				];
			} else {
				$res = [
					'kode'		=> 'error',
            ...
            $res = [
                'kode'		=> 'error',
                'msg'		=> 'Surat Masuk Gagal Diperbaiki'
            ];
		}
		return $res;
    }
